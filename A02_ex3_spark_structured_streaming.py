...
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, current_time, seconds_horizon):
    ...
    # 3.1. We define the Schema of our SDF.
    ...
    ).withColumn("time", f.col("date")).withColumn("stop", f.col("closerStopID")).drop("date", "busLineID", "busLinePatternID", "congestion", "longitude", "latitude", "delay", "closerStopID", "atStop")
...
def streaming_simulation(local_False_databricks_True,
    ...
    if (len(dataset_file_names) > 0):
        ...
        if local_False_databricks_True == False:
            shutil.copyfile(source_dir + file, monitoring_dir +
                            file[:-4] + "_redundant.csv")
        else:
            dbutils.fs.cp(source_dir + file, monitoring_dir +
                          file[:-4] + "_redundant.csv")
...
def my_main(spark,
    ...
    dsw = my_model(spark, monitoring_dir, checkpoint_dir,
                   time_step_interval, current_time, seconds_horizon)
