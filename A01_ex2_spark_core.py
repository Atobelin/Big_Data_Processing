...
def my_main(sc, my_dataset_dir, vehicle_id):
    ...
    filteredRDD = allRDD.filter(lambda x: x[7] == vehicle_id)
    pairRDD = filteredRDD.map(my_pair)
    resultRDD = pairRDD.groupByKey().mapValues(my_group).sortByKey()
    max = resultRDD.max(lambda x: len(x[1]))
    solutionRDD = resultRDD.filter(lambda x: len(x[1]) == len(max[1]))
    solutionRDD.persist()
    # ---------------------------------------
