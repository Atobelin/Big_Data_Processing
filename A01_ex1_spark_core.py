...
def my_main(sc, my_dataset_dir, bus_stop, bus_line, hours_list):
    ...
    filteredRDD = allRDD.filter(lambda x: my_filter(x, bus_stop, bus_line, hours_list))
    pairRDD = filteredRDD.map(my_pair)
    solutionRDD = pairRDD.groupByKey().mapValues(my_group).sortBy(lambda x: x[1])
    solutionRDD.persist()
