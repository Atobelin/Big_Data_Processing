...
def my_model(spark, monitoring_dir, checkpoint_dir, time_step_interval, bus_stop, bus_line, hours_list):
    ...
    withHourSDF = filteredSDF.withColumn("hour", hour_udf(f.col("date")))
    solutionSDF = (
        withHourSDF.groupBy(["hour"])
        .agg({"delay": "avg"})
...
def my_main(spark,
    ...
    dsw = my_model(spark, monitoring_dir, checkpoint_dir,
                   time_step_interval, bus_stop, bus_line, hours_list)
