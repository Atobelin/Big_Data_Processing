...
def my_filter(x, bus_stop, bus_line, hours_list):
    return (
        is_workday(x[0].split(" ")[0])
        and x[1] == bus_line
        and x[8] == bus_stop
        and x[9] == 1
        and x[0].split(" ")[1].split(":")[0] in hours_list
    )
...
# ------------------------------------------
def my_pair(x):
    return (x[0].split(" ")[1].split(":")[0], x[6])
...
def my_state_update(time_interval_list_of_collected_new_values, cur_agg_val):
    if (cur_agg_val is None):
        return time_interval_list_of_collected_new_values

    return time_interval_list_of_collected_new_values + cur_agg_val
...
def my_model(ssc, monitoring_dir, bus_stop, bus_line, hours_list):
    ...
    # TO BE COMPLETED
    # ---------------------------------------
    allDStream = inputDStream.map(process_line)
    filteredDStream = allDStream.filter(
        lambda x: my_filter(x, bus_stop, bus_line, hours_list))
    pairDStream = filteredDStream.map(my_pair)
    updatedDStream = pairDStream.updateStateByKey(my_state_update)
    solutionDStream = updatedDStream.mapValues(
        my_group).transform(lambda rdd: rdd.sortBy(lambda x: x[1]))
    ...
    for file in dataset_file_names:
        ...
        if (verbose == True):
            print("File " + str(count) +
                  " transferred. Time since start = " + str(time.time() - start))
...
def create_ssc(sc, time_step_interval, monitoring_dir, bus_stop, bus_line, hours_list):
    ...
    my_model(ssc, monitoring_dir, bus_stop, bus_line, hours_list)
...
def my_main(sc,
    ...
    dataset_file_names = get_source_dir_file_names(
        local_False_databricks_True, source_dir, verbose)
...
if __name__ == '__main__':
    ...
    source_dir = "FileStore/tables/6_Assignments/A02_ex1_micro_dataset/"
