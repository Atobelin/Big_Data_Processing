...
def my_pair(x):
    return (x[0].split(" ")[0].split("-")[2], x[1])
...
def my_model(ssc, monitoring_dir, vehicle_id):
    ...
    filteredDStream = allDStream.filter(lambda x: x[7] == vehicle_id)
    pairDStream = filteredDStream.map(my_pair)
    resultDStream = pairDStream.groupByKey().mapValues(
        my_group).transform(lambda rdd: rdd.sortByKey())
    solutionDStream = resultDStream.reduce(
        lambda x, y: x if len(x[1]) > len(y[1]) else y)
