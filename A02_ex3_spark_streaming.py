...
# ------------------------------------------
def vehicle_filter(x, current_time, seconds_horizon):
    if x[9] == 1:
        c_time = date_string_to_second(current_time)
        x_time = date_string_to_second(x[0])
        return x_time >= c_time and x_time < c_time + seconds_horizon
...
def my_model(ssc, monitoring_dir, current_time, current_stop, seconds_horizon):
    ...
    minDStream = filteredDStream.reduce(
        lambda x, y: x if date_string_to_second(x[0]) < date_string_to_second(y[0]) else y).map(lambda x: (x[7], "min"))

    solutionDStream = allDStream.filter(
        lambda x: vehicle_filter(x, current_time, seconds_horizon)
    ).map(lambda x: (x[7], (x[0], x[8]))).join(minDStream).filter(lambda x: x[1][1] == "min").map(lambda x: (x[0], x[1][0]))
    # ---------------------------------------
...
if __name__ == '__main__':
    ...
    source_dir = "FileStore/tables/6_Assignments/A02_ex3_micro_dataset/"
